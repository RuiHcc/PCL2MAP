...
int main(int argc, char** argv)
{
  ...
  //  private_nh.param("file_directory", file_directory, std::string("/home/hr/rosbag/"));//此处需要修改为自己pcd文件的路径
  //  ROS_INFO("*** file_directory = %s ***\n", file_directory.c_str());
 
 
  //  private_nh.param("file_name", file_name, std::string("1"));//此处"pcd_name"需要修改为自己的pcd文件名，无需.pcd
  ...
  //  private_nh.param("map_topic_name", map_topic_name, std::string("map"));
   ...
   ros::Subscriber pcl_sub = nh.subscribe<sensor_msgs::PointCloud2>("/mapping_ant01/global_pcl", 5, cloudCallback);
